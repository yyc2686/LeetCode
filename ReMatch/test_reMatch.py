...
# @File    : test_reMatch.py
# @Software: PyCharm
"""
    __project_ = LeetCode
    __file_name__ = test_reMatch
    __author__ = yeyuc
...
class TestSolution(unittest.TestCase):
    def setUp(self):
        self.s = ["aa", "aa", "ab", "aab", "mississippi"]
        self.p = ["a", "a*", ".*", "c*a*b", "mis*is*p*."]
        self.assert_result = [False, True, True, True, False]

    def test_threeSum(self):
        s = Solution()
        for i in range(len(self.assert_result)):
            result = s.isMatch(s=self.s[i], p=self.p[i])
