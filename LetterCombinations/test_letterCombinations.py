...
# @File    : test_letterCombinations.py
# @Software: PyCharm
"""
    __project_ = LeetCode
    __file_name__ = test_letterCombinations
    __author__ = yeyuc
...
import unittest
from .LetterCombinations import Solution
...
class TestSolution(unittest.TestCase):
    ...
    def setUp(self):
        self.input = ["23"]
        self.assert_result = [["ad", "ae", "af", "bd", "be", "bf", "cd", "ce", "cf"]]

    def test_letterCombinations(self):
        s = Solution()
        for i in range(len(self.string)):
            result = s.letterCombinations(s=self.input[i])
